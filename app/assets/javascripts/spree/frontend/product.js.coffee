Spree.ready ($) ->
  Spree.addImageHandlers = ->
    ...
    thumbnails.find('li').on 'mouseenter', (event) ->
      ($ '#main-image img').attr 'src', ($ event.currentTarget).find('a').attr('href').replace('product/', 'large/')
      
      ($ '#main-image').data 'selectedThumb', ($ event.currentTarget).find('a').attr('href')
      ($ '#main-image').data 'selectedThumbId', ($ event.currentTarget).attr('id')
      thumbnails.find('li').removeClass 'selected'
      ($ event.currentTarget).addClass 'selected'
      false

    thumbnails.find('li').on 'mouseleave', (event) ->
      ($ '#main-image img').attr 'src', ($ '#main-image').data('selectedThumb').replace('product/', 'large/')
