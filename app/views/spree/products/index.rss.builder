  ...
  xml.channel{
    ...
    @products.each do |product|
      xml.item do
        ...
        xml.description(simple_format(((product.description.nil? || product.description == "") ? Spree.t(:product_has_no_description) : product.description)))      
        ...
        if product.images.count > 0
          xml.tag!('g:image_link', Spree::Store.current.url + product.images.first.attachment.url(:large))
        elsif product.variant_images.count > 0
          xml.tag!('g:image_link', Spree::Store.current.url + product.variant_images.first.attachment.url(:large))
        end
        ...
        unless product.master.prices.take.offer_price.nil?
