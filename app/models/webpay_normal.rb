...
class WebpayNormal
    ...
    def initTransaction(amount, buyOrder, sessionId, urlReturn, urlFinal)
      ...
      Spree::Webpay.log('init_transaction', 'buyorder:' + buyOrder , 'request', document.to_s, sessionId)
      begin
        ...
        Spree::Webpay.log('init_transaction',  'buyorder:' + buyOrder, 'response', response_array.to_s, sessionId)
      ...
      Spree::Webpay.log('init_transaction',  'token: ' + token.to_s , 'response', response_array.to_s, sessionId)
      return response_array
    end
    ...
    def getTransactionResult(token)
      ...
      #firmar la peticion
      document = sign_xml(req)
      Spree::Webpay.log('get_transaction_result', 'token: ' + token.to_s , 'request', document.to_s, token)
      ...
      begin
        ...
        Spree::Webpay.log('get_transaction_result', 'token: ' + token.to_s , 'response', response_array.to_s, token)
        return response_array
      end
      ...
      Spree::Webpay.log('get_transaction_result', 'token: ' + token.to_s , 'response', response_array.to_s, response_document)
    ...
    def acknowledgeTransaction(token)
      ...
      document = sign_xml(req)
      Spree::Webpay.log('acknowledge_transaction', 'token: ' + token.to_s , 'request', document.to_s, 'null')
      ...
      begin
        ...
        Spree::Webpay.log('acknowledge_transaction', 'token: ' + token.to_s , 'request', 'null', token)
      rescue Exception, RuntimeError => e
        puts "Ocurrio un error en la llamada a Webpay: "+e.message
        response_array ={
            "error_desc" => "Ocurrio un error en la llamada a Webpay: "+e.message
        }
        Spree::Webpay.log('acknowledge_transaction', 'token: ' + token.to_s , 'response', response_array.to_s, 'null')
        return response_array
      end
