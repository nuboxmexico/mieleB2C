  ...
  class Webpay
    #Normal transacition
     def self.init_transaction(order, base_url)
        ...
        order.update(tbk_transaction_id: sessionId)
        return @webpay.getNormalTransaction.initTransaction(amount, order.number, sessionId, @urlReturn, @urlFinal)
     end
      ...
      begin
        ...
        File.open(directory, 'a+') do |f|
          if type.to_s == "request" && action.to_s == "init_transaction"
            ...
            #f << "\t [commerceId] => " + Rails.application.secrets.webpay_commerce_code.to_s + "\n"
            f << "\t [buyOrder] => " + id.to_s.split(':')[1].to_s + "\n"
            f << "\t [sessionId] =>" + sessionId.to_s + "\n"
            f << "\t [returnURL] => #{Rails.application.config.action_mailer.asset_host}" + Rails.application.secrets.webpay_return_url.to_s + "\n"
            f << "\t [finalURL] #{Rails.application.config.action_mailer.asset_host}" + Rails.application.secrets.webpay_final_url.to_s + "\n"
          ...
          #f << time.to_s + "\t" + action.to_s + "\t" + id.to_s + "\t" + type.to_s + "\t" + payload_text.to_s + "\n" + "\n"
        end
      rescue
        puts 'Ha ocurrido un error guardando los logs de WebPay'
      end
